...
class GraphNorm(nn.Module):
    ...
    def forward(self,node_emb,graph):
        ...
        node_mean = scatter(node_emb, graph.batch, dim=0, dim_size=graph._num_graphs, reduce='mean')
        node_mean = node_mean.repeat_interleave(num_nodes_list, 0)

        sub = node_emb - node_mean*self.scale
        node_std = scatter(sub.pow(2), graph.batch, dim=0, dim_size=graph._num_graphs, reduce='mean')
