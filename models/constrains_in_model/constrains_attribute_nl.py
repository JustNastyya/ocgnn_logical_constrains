...
class NodeOCGINAdditionalArgument(nn.Module):
...
def train_node_ocgin_constrains_attribute(model, data, train_mask, test_mask, epochs, lr, constrains_obj):
    optimizer = torch.optim.Adam(model.parameters(), lr=lr, weight_decay=1e-5)

    L_constrains = constrains_obj.get_constraint_value(data)
    L_constrains = L_constrains.unsqueeze(1)
    data.x = torch.cat([data.x, L_constrains], dim=1)
